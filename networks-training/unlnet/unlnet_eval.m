function res = unlnet_eval(net, x, dzdy, res, varargin)
...
%
%   DPRNET layer:
%     The DPRNET layer wraps dprnet(). It has fields:
%
%     - `layer.type` contains the string `'dprnet'.
...
for i=1:n
  ...
  switch l.type
    ...
    case 'unlnet'
      ...
      if doder
        [res(i+1).x,~,res(i+1).aux{1},res(i+1).aux{2}] = unlnet( ...
          checkInput(res(i).x), opts.netParams.Obs, l.weights, ...
          l.rbf_means, l.rbf_precision, opts.netParams.stdn, [], ...
          'conserveMemory', false, 'stride', l.stride, 'padSize', ...
          l.padSize, 'padType', l.padType, 'zeroMeanFilters', ...
          l.zeroMeanFilters, 'weightNormalization', ...
          l.weightNormalization, 'cuDNN',cudnn{:}, 'data_mu', ...
          opts.netParams.data_mu,'step',opts.netParams.step, ...
          'origin',opts.netParams.origin, 'shrink_type',l.shrink_type, ...
          'lb', l.lb, 'ub', l.ub, 'Nbrs_idx', opts.netParams.Nbrs_idx);
      else
        res(i+1).x = unlnet(checkInput(res(i).x), opts.netParams.Obs, ...
          l.weights, l.rbf_means, l.rbf_precision, opts.netParams.stdn, ...
          [], 'conserveMemory', true, 'stride', l.stride, 'padSize', ...
          l.padSize, 'padType', l.padType, 'zeroMeanFilters', ...
          l.zeroMeanFilters, 'weightNormalization', ...
          l.weightNormalization, 'cuDNN',cudnn{:}, 'data_mu', ...
          opts.netParams.data_mu,'step',opts.netParams.step, ...
  ...
  for i=n:-1:backPropLim
    ...
    switch l.type
      ...
      case 'unlnet'
        ...
        input = {res(i+1).x{3}{2},{res(i+1).x{3}{1},res(i+1).x{2}},...
          res(i+1).x{1}{2},{res(i+1).x{1}{1},checkInput(res(i).x)}};
        res(i+1).x(1:3) = [];
        
        [res(i).dzdx,dzdw] = unlnet(input, opts.netParams.Obs, l.weights, ...
          l.rbf_means, l.rbf_precision, opts.netParams.stdn, res(i+1).dzdx, ...
          'stride', l.stride, 'padSize', l.padSize, 'padType', l.padType, ...
          'zeroMeanFilters',l.zeroMeanFilters, 'weightNormalization', ...
          l.weightNormalization, 'cuDNN', cudnn{:}, 'Jacobian', ...
          res(i+1).aux{1}, 'clipMask', res(i+1).aux{2}, 'learningRate', ...
          l.learningRate, 'first_stage', l.first_stage, 'data_mu', ...
