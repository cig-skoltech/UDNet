function net = unlnet_greedy_train(net,varargin)
...
opts.expDir = fullfile('Results', ...
  sprintf('%s-stages:%.0f-psize:%.0fx%.0f@%d%s%s-std:%s-solver:%s-greedyTrain',...
...
for stage = 1:numStages
  ...
  start_time = tic;
  net_ = deep_net_train(net_, imdb, @(x,y)getBatch(x,y,Nbrs_idx), ...
    'expDir', expDir_stage, ...
    'solver', opts.solver, ...
  ...
  if stage < numStages
    ...
    for t = 1:opts.batchSize_val:N
      ...
      Nidx = Nbrs_idx(:,:,:,batchStart:batchEnd);
      ...
      if stage == 1                
        res=opts.net_eval(net_,obs,[],[],'conserveMemory',true, ...
          ...
          opts.data_mu,'stdn',noise_std,'Obs',[],'Nbrs_idx', Nidx));
      else
        res=opts.net_eval(net_,input,[],[],'conserveMemory',true, ...
          'cudnn', opts.cudnn, 'netParams', struct('data_mu', ...
          opts.data_mu,'stdn',noise_std,'Obs', obs, 'Nbrs_idx', Nidx));
      end
...
aux = struct('Obs',imdb.images.obs(:,:,:,batch), ...
             'stdn',imdb.images.noise_std(ceil(batch/N_gt)), ...
