function net = unet_add_layer(net,varargin)
...
switch opts.layer_type
  ...
  case {'unet'}
    ...
    if nlr~=6
      opts.learningRate = [opts.learningRate(:)',ones(1,6-nlr,'like',opts.learningRate)];
    end
    
    if opts.weightSharing
      opts.ht = [];
      opts.st = [];
      opts.learningRate([2,4])=0; % We don't learn any independent weights for the conv2Dt layers      
    end          
    
    if isempty(opts.s), opts.learningRate(3) = 0; end
    if isempty(opts.st), opts.learningRate(4) = 0; end    
    ...
    net.layers{end+1} = struct('type', opts.layer_type, ...
      ...
      'weights', {{opts.h, opts.ht, opts.s, opts.st, ...
