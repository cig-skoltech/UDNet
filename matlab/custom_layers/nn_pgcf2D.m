function [y,dh,dsh,dg] = nn_pgcf2D(x,h,sh,g,idx,dzdy,varargin)

% NN_PGCF2D (Patch Group Collaborative Filtering)
%
...
%
%   If ConserveMemory is set to false (see below) then Y is a 2 x 1 cell
%   array which keeps the outputs of some of the internal layers, necessary
%   for the backward mode.
%
%   [DZDX,DH,DSH,DG,DSG] = NN_PGCF2D(X, H, SH, G, SG, idx, DZDY)
...
%
%   NN_PGCF2D(..., 'option', value, ...) takes the following options:
%
%   `Stride`:: 4
...
if numel(g) ~= opts.Nbrs
  error(['nn_pgcf2D:: The number of the neighbor-filter coefficients ' ...
...
if (isempty(dzdy) || nargin < 6)
  ...
  patchDims = max(floor(([Nx+sum(opts.padSize(1:2)),...
    Ny+sum(opts.padSize(3:4))]-patchSize)./opts.stride)+1,0); %[H',W']
  % Number of spatial filters
  
  if (size(idx,1) ~= patchDims(1) || size(idx,2) ~= patchDims(2) || ...
      size(idx,3) ~= opts.Nbrs || size(idx,4) ~= NI)
    error('nn_pgcf2D:: idx does not have the correct dimensions.');
  end
  
  if opts.conserveMemory
    % Spatial convolution - y is of size H' x W' x numFilters x NI
    y = nn_conv2D(x,h,[],sh,[],'stride',opts.stride,'padSize',...
    ...
    if useGPU
      y = FMapNLSum_gpu(y,g,idx-1);
    else
      y = FMapNLSum_cpu(y,g,idx-1);
    end
    
  else
    y = cell(2,1);
    
    % Spatial convolution - y{1} is of size H' x W' x numFilters x NI
    y{1} = nn_conv2D(x,h,[],sh,[],'stride',opts.stride,'padSize',...
      opts.padSize,'padType',opts.padType,'cuDNN',opts.cuDNN,...
      'zeroMeanFilters',opts.zeroMeanFilters,'weightNormalization',...
      opts.weightNormalization(1));
    
    if useGPU
      y{2} = FMapNLSum_gpu(y{1},g,idx-1);
    else
      y{2} = FMapNLSum_cpu(y{1},g,idx-1);
    end
  ...
  if isa(dzdy,'gpuArray')
    ...
    y = FMapNLSumT_gpu(dzdy,g,idx-1,n,I-1);
  else
    y = FMapNLSumT_cpu(dzdy,g,idx-1,n,I-1);
  end
  
  
  derParams = [opts.derParams(1) false opts.derParams(2)];
  [y,dh,~,dsh] = nn_conv2D(x{2},h,[],sh,y,'stride',opts.stride,...
